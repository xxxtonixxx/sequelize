...
const QueryGenerator = {
  ...
  */
  ...
  getQueryOrders(options, model, subQuery) {
    ...
    } else if (options.order instanceof Utils.Literal) {
      if (subQuery) {
        subQueryOrder.push(options.order.val);
      }
      mainQueryOrder.push(options.order.val);
    } else {
      let err = 'Expected array or Sequelize.literal() for order, got ' + util.inspect(options.order);
      if (typeof options.order === 'string') {
        err = 'String order has been removed in v4. ' + err;
      }
      throw new Error(err);
    }
